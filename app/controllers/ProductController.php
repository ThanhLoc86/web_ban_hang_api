...
class ProductController
{
    ...
    public function addToCart($id)
    {
        ...
        header('Location: /web_ban_hang/Product/cart');
    }
    ...
    public function processCheckout()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                header('Location: /web_ban_hang/Product/orderConfirmation');
            } catch (Exception $e) {
    ...
    public function updateQuantity($product_id) {
        // //Lấy số lượng mới từ POST request
        // $new_quantity = $_POST['quantity'];
    
        // // Cập nhật giỏ hàng (giả sử giỏ hàng lưu trữ trong session)
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Kiểm tra nếu số lượng hợp lệ và sản phẩm có trong giỏ hàng
            ...
            // Trả về JSON để cập nhật số lượng trên giao diện mà không cần tải lại
            echo json_encode(["success" => true, "totalPrice" => $totalPrice]);
            exit();
        }
    
        // Nếu không phải phương thức POST, quay lại trang giỏ hàng
        header("Location: /web_ban_hang/Cart");
        exit(); 
    }
